		...
		if (c != null) {
			if (c.moveToFirst()) {
				do {
					...
					Log.w("fl-laCObser","next cal = " + cal.get(Calendar.YEAR)+"-"+(cal.get(Calendar.MONTH)+1)+"-"+cal.get(Calendar.DAY_OF_MONTH)+" "+cal.get(Calendar.HOUR_OF_DAY)+":"+cal.get(Calendar.MINUTE));
					FlashLightSensorListener.getInstance().addNextAlarmClock(cal);
